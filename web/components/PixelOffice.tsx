    ...
    <div className="flex flex-col items-center gap-1 relative">
      {isActive && bubbleText && (
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 whitespace-nowrap z-10">
          <div className="bg-slate-800 border border-slate-600 rounded px-2 py-0.5 text-[9px] text-slate-300 max-w-[180px] truncate"
      ...
      <div className="text-[8px] font-mono px-1 rounded mb-0.5"
           style={{ backgroundColor: isActive ? c.accent : "#1e293b", color: isActive ? "#fff" : "#64748b",
  ...
  return (
    <div className="bg-slate-900 rounded-lg border border-slate-800 p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Agent Office</h2>
      ...
      <div className="flex items-end justify-center gap-6 sm:gap-10 py-4 px-2 relative">
        <div className="absolute bottom-0 left-0 right-0 h-2 bg-slate-800 rounded" />
        {["planner", "executor", "reviewer"].map((role) => (
          <DeskRow key={role} role={role} isActive={isRunning && activeRole === role}
